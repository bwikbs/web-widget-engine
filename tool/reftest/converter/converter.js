...
var pathToSave = "test/reftest/csswg-test_converted/" + args[1] + "/";
...
page.onLoadFinished = function() {
    //console.log("INJECT:"+page.injectJs('tool/pixel_test/cssParser.js'));
    page.injectJs('tool/reftest/converter/cssParser.js');
    resEvaluate = page.evaluate(function(acceptCSSList, acceptTagList, acceptValues, includes, startsWith) {
        ...
        var styleFiltering = function() {
            ...
            for (var i = 0; i < styleTags.length; i++) {
                ...
                var parseResult = parser.parse(styleTags[i].textContent, true, true);
                if (!parseResult) {
                    console.log("[parseResult is empty] " + styleTags[i].textContent);
                    continue;
                }
...
//                    console.log(rules[j].parsedCssText);
                    var decl = rules[j].declarations;
                    if (decl) {
                        var len = decl.length;
                        for (var k = 0; k < len; k++) {
//                            console.log(decl[k].valueText);
                            if (!decl[k].property) continue;
                            else if (decl[k].property == "transform") {
                            ...
                            } else if (decl[k].property == "transform-origin") {
                                var d = new jscsspDeclaration();
                                d.property = "-webkit-transform-origin";
                                d.valueText = decl[k].valueText;
                                d.parsedCssText = d.property + ": " + d.valueText + ";";
                                d.values = decl[k].values;
                                rules[j].declarations.push(d);
                            } else if (decl[k].property.startsWith("border-image-")) {
                                var d = new jscsspDeclaration();
                                d.property = "-webkit-"+d.property;
                                d.valueText = decl[k].valueText;
                                d.parsedCssText = d.property + ": " + d.valueText + ";";
                                d.values = decl[k].values;
                                rules[j].declarations.push(d);
                            } else if (decl[k].property == "font") {
                                decl[k].valueText = "//"+decl[k].valueText;
                                decl[k].parsedCssText = "//" + decl[k].parsedCssText;
                                decl[k].values = [];
                            }
                        }
                    }
                    if (!sel || /^[a-zA-Z0-9_]+$/.test(sel) || /^[#.][a-zA-Z0-9_]+$/.test(sel)) {
                    ...
                    try {
                        ...
                        console.log("[SELECTOR: " + sel + "] " + err);
                        rules[j].mSelectorText = sel;
                        styleNum--;
                        elems = {length: 0};
                    }
                    styleTxt += rules[j].cssText() + "\n";
...
//            console.log("(result)"+document.result);
    ...
    }, acceptCSSList, acceptTagList, acceptValues, Array.prototype.includes, String.prototype.startsWith);
...
setInterval(function() {
    ...
    if (resEvaluate != undefined) {
        var newfilename = filelist[idx].replace("csswg-test", "csswg-test_converted");
        // pathToSave + filelist[idx].substring(filelist[idx].indexOf("csswg-test") + String("csswg-test").length + 1);
