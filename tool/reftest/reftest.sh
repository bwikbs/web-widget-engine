...
function main {
    ...
    if [[ "$1" = *"dom_conformance_test.res" ||
        ...
        doTest "$@"
    # HTML input file is not supported
    elif [[ "$1" = *"webkit_fast_css.res" ]]; then
        TESTSUITE=2
        TESTSUITENAME="WebKit Fast CSS - Pixel Test"
        #PASSFILE="test/regression/tool/vendor/webkit/test_webkit_fast_css"
        doTest "$@"
    elif [[ "$1" = *"webkit_fast_css_manual.res" ]]; then
        TESTSUITE=3
        TESTSUITENAME="WebKit Fast CSS - Regression Test"
        #PASSFILE="test/regression/tool/vendor/webkit/test_webkit_fast_css"
        doTest "$@"
...
function doTest {
    ...
    for i in $tc ; do
        ...
        for c in $(seq 0 $cnt); do
            ...
            # Collect the result
            if [ `expr $TESTSUITE \< 2` -eq 1 ] || [ $TESTSUITE -eq 4 ] ; then
                replace1='s/\\n"//g'
                replace2='s/"//g'
                replace3='s/\n//g'
