...
namespace rapidxml
{
    ...
    const int print_no_expand_lt_gt = 0x1 << 1;
    const int print_no_expand_quot = 0x1 << 2;
    const int print_no_expand_amp = 0x1 << 3;
    const int print_care_script_style = 0x1 << 4;
    ...
    namespace internal
    {
        ...
        template<class OutIt, class Ch>
        inline OutIt copy_and_expand_chars(const Ch *begin, const Ch *end, Ch noexpand, OutIt out, int flags = 0)
        {
            while (begin != end)
            {
                if (*begin == noexpand)
                ...
                else
                {
                    ...
                    {
                    case Ch('<'):
                        if (!(flags & print_no_expand_lt_gt)) {
