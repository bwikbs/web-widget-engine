...
#undef BORDER_STYLE
    ...
    // backgroundSize
    {
        ...
        if (m_style->bgSizeType() == BackgroundSizeType::Cover) {
            ...
            Length w = m_style->bgSizeValue()->width();
            Length h = m_style->bgSizeValue()->height();

            if (w.isFixed()) {
                vals->append(CSSStyleValuePair::ValueKind::Length, CSSLength(w.fixed()));
            } else if (w.isPercent()) {
                vals->append(CSSStyleValuePair::ValueKind::Percentage, w.percent());
            } else if (w.isAuto()) {
                vals->append(CSSStyleValuePair::ValueKind::Auto, 0);
            } else {
                STARFISH_ASSERT(false);
            }

            if (h.isFixed()) {
                vals->append(CSSStyleValuePair::ValueKind::Length, CSSLength(h.fixed()));
            } else if (h.isPercent()) {
                vals->append(CSSStyleValuePair::ValueKind::Percentage, h.percent());
            } else if (h.isAuto()) {
                vals->append(CSSStyleValuePair::ValueKind::Auto, 0);
            } else {
                STARFISH_ASSERT(false);
